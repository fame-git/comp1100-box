...
public class RBTree<T extends Comparable<T>> {
	...
	public boolean testProp3() {
		// START YOUR CODE
		Set<Integer> numBlackNodes = new HashSet<>();
		testProp3Helper(numBlackNodes, 0, this.root);
		return numBlackNodes.size() == 1; //you are allowed to change this return statement
		// END YOUR CODE
	}

	public void testProp3Helper(Set<Integer> numBlackNodes, Integer currentCount, Node node) {
		Integer thisCount = currentCount;

		if (node.colour == Colour.BLACK) {
			thisCount++;
		}
		if (node.left == null && node.right == null) {
			numBlackNodes.add(currentCount);
		} else {
			if (node.left != null && node.right != null) {
				testProp3Helper(numBlackNodes, thisCount, node.left);
				testProp3Helper(numBlackNodes, thisCount, node.right);
