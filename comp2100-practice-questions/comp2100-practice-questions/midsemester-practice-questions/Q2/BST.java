...
public class BST {
	...
	public Integer evenNodeSum() {
		// START YOUR CODE
		return evenNodeSumHelper(this.root); //you are allowed to change this return statement
		// END YOUR CODE
	}

	public Integer evenNodeSumHelper (Node node) {
		if (node.left == null && node.right == null) {
			return node.value;
		} else {
			if (node.left != null && node.right != null) {
				return node.value + evenNodeSumHelper(node.left) + evenNodeSumHelper(node.right);
			} else if (node.left != null) {
				return evenNodeSumHelper(node.left);
			} else {
				return evenNodeSumHelper(node.right);
			}
		}
	}
