...
public class AVLTree<T extends Comparable<T>> extends BinarySearchTree<T> {
    ...
    public AVLTree<T> insert(T element) {
         ...
         */
        AVLTree<T> result = new AVLTree<>(value,leftNode,rightNode);
        ...
        if (element.compareTo(value) > 0) {
            // COMPLETE
            result.rightNode = rightNode.insert(element);
        } else if (element.compareTo(value) < 0) {
            // COMPLETE
            result.leftNode = leftNode.insert(element);
        } else {
            //return new AVLTree<>(value, leftNode, rightNode);// COMPLETE
            return this;
        }

        if(result.getBalanceFactor() == 2){
            if (((AVLTree<T>) result.leftNode).getBalanceFactor() < 0)
                result.leftNode = ((AVLTree<T>) result.leftNode).leftRotate();
            return result.rightRotate();
        }
        else if (result.getBalanceFactor() == -2){
            if(((AVLTree<T>) result.rightNode).getBalanceFactor() > 0)
                result.rightNode = ((AVLTree<T>) result.rightNode).rightRotate();
